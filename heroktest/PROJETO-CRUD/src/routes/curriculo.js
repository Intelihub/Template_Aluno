...
router.all("/", (req, res) => {
	...
	const sql1 = "SELECT idCurriculo, item, descricao, nomeCategoria \
	FROM curriculo  \
	INNER JOIN categoria ON  curriculo.idCategoria = categoria.idCategoria WHERE curriculo.idCategoria = 1 ";
	const sql2 = "SELECT idCurriculo, item, descricao, nomeCategoria \
	FROM curriculo  \
	INNER JOIN categoria ON  curriculo.idCategoria = categoria.idCategoria WHERE curriculo.idCategoria = 2 ";

	db.all(sql1, params, (err, rows1) => {
		if (err) {
		...
		db.all(sql2, params, (err2, rows2) => {
			if (err2) {
				console.error(err2.message);
				res.send("Erro: " + err2.message);
				return;
			}

			res.render("curriculo/listar", { model1: rows1, model2: rows2 });
		});
	});
});

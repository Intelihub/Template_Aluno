...
router.all("/listar", (req, res) => {
	...
	db.all(sql, params, (err, rows) => {
		...
		res.render("escola/listar", { model: rows });
	});
});

router.all("/notas", (req, res) => {
	let pessoas;
	let ordenar = req.query["ordenar"];
	let params;
	...
	const sql = "SELECT idEstudante, nomeCompleto, nomeEscola  \
				 FROM estudante AS s \
				 INNER JOIN escola as e ON s.idEscola = e.idEscola " 
				  + ordenar;
	console.log(sql);
